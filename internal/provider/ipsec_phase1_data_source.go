...
var (
	_ datasource.DataSource              = (*IPsecPhase1DataSource)(nil)
	_ datasource.DataSourceWithConfigure = (*IPsecPhase1DataSource)(nil)
)

type IPsecPhase1DataSourceModel struct {
	IPsecPhase1Model
}

func NewIPsecPhase1DataSource() datasource.DataSource { //nolint:ireturn
	return &IPsecPhase1DataSource{}
}

type IPsecPhase1DataSource struct {
	client *pfsense.Client
}

func (d *IPsecPhase1DataSource) Metadata(_ context.Context, req datasource.MetadataRequest, resp *datasource.MetadataResponse) {
	resp.TypeName = fmt.Sprintf("%s_ipsec_phase1", req.ProviderTypeName)
}

func (d *IPsecPhase1DataSource) Schema(_ context.Context, _ datasource.SchemaRequest, resp *datasource.SchemaResponse) {
	resp.Schema = schema.Schema{
		Description:         "Retrieves a single IPsec Phase 1 tunnel configuration by IKE ID.",
		MarkdownDescription: "Retrieves a single IPsec [Phase 1](https://docs.netgate.com/pfsense/en/latest/vpn/ipsec/configure.html) tunnel configuration by IKE ID.",
...
func (d *IPsecPhase1DataSource) Configure(_ context.Context, req datasource.ConfigureRequest, resp *datasource.ConfigureResponse) {
...
func (d *IPsecPhase1DataSource) Read(ctx context.Context, req datasource.ReadRequest, resp *datasource.ReadResponse) {
	var data IPsecPhase1DataSourceModel
	...
	p1, err := d.client.GetIPsecPhase1(ctx, data.IKEId.ValueString())
	if addError(&resp.Diagnostics, "Unable to get IPsec phase 1", err) {
		return
	}
